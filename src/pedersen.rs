...
use super::Error::{self, InvalidCommit, InvalidPublicKey};
use Secp256k1;
...
impl Commitment {
    ...
    pub fn from_pubkey<C: Verification>(secp: &Secp256k1<C>, pk: &key::PublicKey) -> Result<Self, Error> {
        unsafe {
            let mut commit_i = [0; constants::PEDERSEN_COMMITMENT_SIZE_INTERNAL];
            if ffi::secp256k1_pubkey_to_pedersen_commitment(
                secp.ctx,
                commit_i.as_mut_ptr(),
                &pk.0 as *const _,
            ) == 1
    ...
    pub fn to_pubkey<C: Verification>(&self, secp: &Secp256k1<C>) -> Result<key::PublicKey, Error> {
...
impl<C: Verification> Secp256k1<C> {
    ...
    pub fn rewind_range_proof(
        ...
        ProofInfo {
            ...
            blinding: SecretKey::from_slice(&[0; constants::SECRET_KEY_SIZE]).unwrap(),
            mlen: mlen,
            min: min,
            max: max,
            exp: 0,
...
#[cfg(test)]
mod tests {
    use super::key::{PublicKey, SecretKey, ONE_KEY, ZERO_KEY};
